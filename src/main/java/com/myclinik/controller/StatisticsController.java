...
public class StatisticsController {
	...
	public String filterData(Model model, @RequestParam(name = "client", required = false) Client client,
			...
			@RequestParam(name = "doneAppointment", required = false) Boolean doneAppointment,
		...
		if (treatment != null)
			appointments = appointments.stream()
					.filter(appointment -> appointment.getTreatment().getId() == treatment.getId())
					.collect(java.util.stream.Collectors.toList());
		if (paidAppointment != null)
			appointments = appointments.stream().filter(appointment -> appointment.getPaid() == paidAppointment)
					.collect(java.util.stream.Collectors.toList());
